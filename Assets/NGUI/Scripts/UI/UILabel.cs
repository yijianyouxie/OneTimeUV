...
public partial class UILabel : UIWidget
{
	public enum Effect
	{
		None,
    ...
    private Font TrueTypeFont
    {
        ...
        {
//#if (!UNITY_EDITOR)
//            RenderEngine.Adapter.IRenderAdapter ir = RenderEngine.Scene.SceneManager.RenderAdapter;
//            if(ir  != null && ir.OnlyFont)
//            {
//                return ResourceManager.Singleton.GetDefaultFont();
//            }
//            else
//#endif
            {
    ...
    [System.NonSerialized] List<int> mLineList = new List<int>();
    ...
    [HideInInspector][NonSerialized] bool atEnable = true;
    public void SetAtEnable( int enable )
    {
        atEnable = enable >= 1;
    }
	/// <summary>
	...
	public override Material material
	{
		get
		{
			...
			if (TrueTypeFont != null) return TrueTypeFont.material;
			return null;
		}
		set
		{
	...
	public UnityEngine.Object ambigiousFont
	{
		get
		{
			return (UnityEngine.Object)mFont ?? (UnityEngine.Object)TrueTypeFont;
		}
		set
		{
...
#if DYNAMIC_FONT
	bool isValid { get { return mFont != null || TrueTypeFont != null; } }
#else
	...
	void ProcessText (bool legacyMode, bool full)
	{
		...
		if (mFinalFontSize > 0)
		{
			...
			for (int ps = mFinalFontSize; ps > 0; --ps)
			{
...
#if DYNAMIC_FONT
                    float fScale = (null == mFont) ? 1f : ((float)mFontSize / mFont.defaultSize) * mScale;
                    NGUIText.fontScale = isDynamic ? mScale : fScale;
#else
                ...
                ////插入特殊字典处理
                //if(Application.isPlaying && GameManager.Instance != null && atEnable)
                //{
                //    //只处理完整符合格式的
                //    if (mText.Length == 6 || mText.Length == 7 || mText.Length == 5)
                //    {
                //        if(mText[0] == '@')
                //        {
                //            int langId = 0;
                //            if(int.TryParse(mText.Substring(1), out langId))
                //            {
                //                mText = LangUtil.GetKey(langId);
                //            }
                //        }
                //    }
                //}
    ...
    void ProcessText2(bool legacyMode, bool full)
    {
        if (!isValid) return;
        ...
        if (mFinalFontSize > 0)
        {
            ...
            for (int ps = mFinalFontSize; ps > 0; --ps)
            {
                ...
                {
                    NGUIText.CalculatePrintedSize2(mProcessedText,ref mLineList);
...
//        if (caret != null) caret.ForceLocal();
//        if (highlight != null) highlight.ForceLocal();
//#endif        
		...
		int startingCaretVerts = (null == caret) ? 0 : caret.verts.size;
		...
		if (highlight != null && start != end)
		{
			...
			NGUIText.PrintCaretAndSelection(text, start, end, (null == caret) ? null : caret.verts, highlight.verts);
		...
		else NGUIText.PrintCaretAndSelection(text, start, end, (null == caret) ? null : caret.verts, null);
    ...
    public void WriteDrawCallBuffer(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
    {
        if (!isValid) return;
        ...
        //Profiler.BeginSample("UpdateNGUIText");
        UpdateNGUIText();
        //Profiler.EndSample();

        NGUIText.tint = col;
        //Profiler.BeginSample("NGUIText.Print");
        ...
        //Profiler.BeginSample("ApplyOffset");
	...
	{
        //Profiler.BeginSample("UILabel:OnFill");
#if (OPTIMISE_GC_SY_20190426)
        if (uig != null)
        {
            if(uig.m_pUIWidget != null)
            {
                ...
                {
                    WriteDrawCallBuffer(verts, uvs, cols);
                    uig.m_vertexnum = 0;
                }
                else
                {
	...
	{
        //Profiler.BeginSample("ApplyShadow");
		Color c = mEffectColor;
		c.a *= finalAlpha;
		Color32 col = (bitmapFont != null && bitmapFont.premultipliedAlphaShader) ? NGUITools.ApplyPMA(c) : c;

	    var addSize = end - start;
        verts.Allocate(verts.size+addSize);
        uvs.Allocate(uvs.size + addSize);
        cols.Allocate(cols.size + addSize);
		for (int i = start; i < end; ++i)
		{
            verts.buffer[verts.size++] = verts.buffer[i];
            uvs.buffer[uvs.size++] = uvs.buffer[i];
            cols.buffer[cols.size++] = cols.buffer[i];
	...
	{
		if (!paused && TrueTypeFont != null) Invalidate(false);
	}

    [ContextMenu("PreviewVerticalText")]
    public void PreviewVerticalText() { /*LangUtil.GetVerticalBeginFromRight(this, mText, height, "");*/ }
}
