...
public class UIAnchor : CacheBehaviour
{
	public enum Side
	{
		BottomLeft,
	...
	public void ScreenSizeChanged () { if (mStarted && runOnlyOnce) Update(); }
	...
	void Update ()
	{
		...
		else if (pc != null)
		{
			if (pc.clipping == UIDrawCall.Clipping.None)
			{
                ...
                float ratio = (mRoot != null) ? (float)mRoot.activeHeight / Screen.height : 1.0f;
                if (ForceSide)
                {
                    ratio *= 0.5f;
