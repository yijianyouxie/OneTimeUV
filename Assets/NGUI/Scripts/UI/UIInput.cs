...
public class UIInput : CacheBehaviour
{
	...
	public enum Validation
	{
		...
		Name,
		Filename,
        Custom1, //非负整数,第一位可以是0
        Custom2, //非负整数,第一位不能是0
        Title,   // 称号  
    }
    ...
    private float selectInterval = 0.3f;
#endif
	...
	public string Validate (string val)
	{
        ...
        if(characterLimit > 0)
        {
            ...
            {
                string tmp = sb.ToString(0, i+1);
                byte[] tmpData = System.Text.Encoding.Default.GetBytes(tmp.ToCharArray());
                if (tmpData.Length > characterLimit)
                {
	...
	{
//#if GAMEDEBUG
//        Games.TLBB.Log.LogSystem.Error("OnSelect,=====isSelected:{0}", isSelected);
//#endif
        if (isSelected)
		{
#if !MOBILE
        ...
        {
#if !UNITY_EDITOR && UNITY_IPHONE
            if (Time.time - selectTime < selectInterval)
            {
                return;
            }
#endif
	...
	{
//#if GAMEDEBUG
//        Games.TLBB.Log.LogSystem.Error("OnSelectEvent,=====mSelectTime:{0}", mSelectTime);
//#endif
...
//            if (mKeyboard != null)
//            {
//                Games.TLBB.Log.LogSystem.Error("OnDeselectEvent,=====mKeyboard.active:{0}",mKeyboard.active);
//            }else
//            {
//                Games.TLBB.Log.LogSystem.Error("OnDeselectEvent,=====mKeyboard is null.");
//            }
//#endif
			if (mKeyboard != null)
			{
	...
	protected virtual void Update ()
	{
        ...
        {
//#if GAMEDEBUG
//            Games.TLBB.Log.LogSystem.Info("Update,======isSelected:{0}", isSelected);
//#endif
            return;
        }
		...
		if (mWaitForKeyboard)
		{
            ...
            {
//#if GAMEDEBUG
//                Games.TLBB.Log.LogSystem.Error("mKeyboard has't open for special reasons.");
//#endif
                return;
            }
			mWaitForKeyboard = false;
...
//                if (mKeyboard != null)
//                {
//                    Games.TLBB.Log.LogSystem.Error("Update,=====mKeyboard.active:{0}",mKeyboard.active);
//                }else
//                {
//                    Games.TLBB.Log.LogSystem.Error("Update,=====mKeyboard is null.");
//                }
//#endif
			}
			else
		...
		if (mKeyboard != null)
		{
			...
			{
//#if GAMEDEBUG
//                Games.TLBB.Log.LogSystem.Error("Update2,=====mKeyboard.done:{0},mKeyboard.active:{1}",mKeyboard.done,mKeyboard.active);
//#endif
	...
	protected virtual void Insert (string text)
	{
		...
		for (int i = 0, imax = text.Length; i < imax; ++i)
		{
            ...
            byte[] rightData = System.Text.Encoding.Default.GetBytes(right.ToCharArray());

            byte[] cData = System.Text.Encoding.Default.GetBytes(c.ToString().ToCharArray());

			//if (characterLimit > 0 && sb.Length + rl >= characterLimit) break; //之前是纯字符数判断
            if (characterLimit > 0 && tmpData.Length + rightData.Length >= characterLimit) break; //改为字节判断

            if (characterLimit > 0 && tmpData.Length + rightData.Length + cData.Length > characterLimit) break; //改为字节判断
	...
	public void Submit ()
	{
		if (NGUITools.GetActive(this))
		{
			...
			if (current == null)
			{
                ...
                //EventManager.SendEvent(DataEvent.InputTouchCount1);
				current = null;
			}
	...
	protected char Validate (string text, int pos, char ch)
	{
        ...
        else if (validation == Validation.Custom1)
        {
            if (ch >= '0' && ch <= '9' && !(text == "0" && ch == '0') && ch != '-')
            {
                return ch;
            }
        }
        else if (validation == Validation.Custom2)
        {
            if (ch >= '0' && ch <= '9' && !(pos == 0 && ch == '0') && ch != '-')
            {
                return ch;
            }
        }
        ...
        else if (validation == Validation.Title)
        {
            int temp = (int)ch;
            if (temp >= 0 && temp <= 255)
                return (char)0;
