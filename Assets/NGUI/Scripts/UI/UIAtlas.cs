...
public class UIAtlas : CacheBehaviour
{
...
#endif
            //2017-10-11 sunyu 不在构造快捷dic,每次遍历查找
            if (false)
			{
    ...
    void OnDestroy()
    {
        if (null != mSprites)
        {
            int spritesCount = mSprites.Count;
            for (int i = 0; i < spritesCount; i++)
            {
                UISpriteData usd = mSprites[i];
                if (null != usd)
                {
                    //Games.TLBB.Util.GameUtil.PoolDelUISpriteData(ref usd);
                    usd = null;
                }
            }
        }
