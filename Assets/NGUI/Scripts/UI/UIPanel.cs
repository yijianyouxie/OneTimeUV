...
{
    static public bool bDebugClose = false;
	/// <summary>
	...
	bool mRebuild = false;
    bool mDistanceSortWidgets = false;
	bool mResized = false;
    ...
    public bool mbLowFrequence = false;
    private float lateupdatelasttime = 0.0f;
	/// <summary>
    ...
    /// <summary>
    /// 是否开启alpha功能(有些面板不需要alpha半透功能，但是该面板下的所有Widget每次都会调用相关接口计算半透值，造成不必要的性能开销，比如名字板)，为不影响原来功能，alpha半透功能默认开启
    /// </summary>
    public bool enableAlphaFunc = true;

    public bool IsHide = false;

    public int HideDelay = -1;
...
#if OPTIMISE_GC_XY_20190123
                list.Sort(CompareFuncComp);
#else
	...
	public override Vector3[] GetSides (Transform relativeTo,bool ForceSide =false)
	{
		if (mClipping != UIDrawCall.Clipping.None)
		{
			float x0 = mClipOffset.x + mClipRange.x - 0.5f * mClipRange.z;
		...
		return base.GetSides(relativeTo, ForceSide);
	}
    ...
    public void DistanceSortWidgets() { mDistanceSortWidgets = true; }
    ...
    //不更新经常罗一块找起来很麻烦，增加个编辑器方法查问题
    [ContextMenu("UpdateAllPanel")]
    void UpdateAllPanelForEditor()
    {
    ...
    public static void LateCustomUpdate ()
	{
        if (bDebugClose)
            return;
        int frameCount = Time.frameCount;
//         if(frameCount - UIRect.uiOpenFrame > 10 && frameCount - mUpdateFrame < 3)
//         {
//             return;
//         }
        //Profiler.BeginSample("UIPanel:LateUpdate");
#if UNITY_EDITOR
...
#endif
		{
            ...
            //Profiler.BeginSample("UIPanel:UpdateSelf");
			// Update each panel in order

            //之前是for (int i = 0, imax = list.Count; i < imax; ++i) 有可能出现越界
            //当scrollbar更新回调onchange函数里修改某个panel的enable属性会导致list数量修改
            //2017-4-20 16:32:18 by chenxiaoyu&liuzhen
            for (int i = 0; i < list.Count; ++i)
            {
                UIPanel p = list[i];
                p.IsHide = p.HideDelay == 0;
                if (p.HideDelay > 0)
                {
                    p.HideDelay--;
                }
            ...
            //Profiler.EndSample();
			int rq = 3000;
            //Profiler.BeginSample("UIPanel:UpdateDrawCalls");
			...
			for (int i = 0, imax = list.Count; i < imax; ++i)
			{
				...
				if (p.renderQueue == RenderQueue.Automatic)
				{
					...
					p.UpdateDrawCalls();
#if USE_AUTO_RENDER_QUEUE
					int offset = p.UpdateRenderQueueForAutoRenders();
					rq += offset;
#endif
	...
	void UpdateSelf ()
	{
        if (mbLowFrequence)
        {
#if OPTIMISE_XY_20190712
            if (GetInstanceID()%2 != Time.frameCount%2)
            {
                return;
            }
#endif

            if (Time.time - lateupdatelasttime < 0.1)
            {
                return;
            }
            lateupdatelasttime = Time.time;
        }

        //Profiler.BeginSample("UpdateTransformMatrix");
		UpdateTransformMatrix();
        //Profiler.EndSample();

        //Profiler.BeginSample("UpdateLayers");
        UpdateLayers();
        //Profiler.EndSample();

        //Profiler.BeginSample("UpdateWidgets");
        UpdateWidgets();
        //Profiler.EndSample();
        ...
        if (mRebuild)
        {
            //Profiler.BeginSample("mRebuild FillAllDrawCalls");
		...
		{
            //Profiler.BeginSample("Not mRebuild FillAllDrawCalls");

            for (int i = 0; i < drawCalls.Count;)
            {
                UIDrawCall dc = drawCalls[i];
#if USE_AUTO_RENDER_QUEUE
				if (dc.isDirty && !dc.IsEmpty && !FillDrawCall(dc))
#else
        ...
        //Profiler.BeginSample("mUpdateScroll");

        if (mUpdateScroll)
		{
            //Profiler.BeginSample("UIPanel:UpdateSelf->UpdateScrollbars");
			mUpdateScroll = false;
			UIScrollView sv = GetComponent<UIScrollView>();
...
#if OPTIMISE_GC_XY_20190123
    static List<UIWidget> widgetsInDrawCall = new List<UIWidget>(256);

    void FillDrawCallBuffers(UIDrawCall dc, int verticesCount)
    {
        dc.verts.Allocate(verticesCount);
        dc.uvs.Allocate(verticesCount);
        dc.cols.Allocate(verticesCount);
        for (int i = 0; i < widgetsInDrawCall.Count; i++)
        {
            var widget = widgetsInDrawCall[i];
            if (widget != null)
            {
                widget.WriteToBuffers(dc.verts, dc.uvs, dc.cols);
            }
        }
    ...
    void FillAllDrawCalls ()
	{
        ...
        if (mDistanceSortWidgets)
        {
            mDistanceSortWidgets = false;
            widgets.Sort(UIWidget.PanelDistanceCompareFunc);
...
//                     continue;
//                 }
//                 if (w.material.shader == null)
//                 {
//                     continue;
//                 }
//                 if (w.material.shader.name == "GUI/Text Shader")
//                     w.material.shader = Shader.Find("GUI/3D NamePad");
//             }
        }
		...
		for (int i = 0; i < widgets.Count; ++i)
		{
			UIWidget w = widgets[i];
#if USE_AUTO_RENDER_QUEUE
			if (w.AutoRenderQueue != null && w.AutoRenderQueue.UseDepth)
            {
#if OPTIMISE_GC_XY_20190123
                ...
                dc = UIDrawCall.Create(this, null, null, null);
                dc.depthStart = w.depth;
                dc.depthEnd = dc.depthStart;
                dc.panel = this;
                dc.IsEmpty = true;
                dc.name = "_DrawCall [Empty For Auto RenderQueue]";
                drawCalls.Add(dc);
                w.drawCall = dc;
                dc = null;
                continue;
			...
			if (w.isVisible && w.hasVertices)
			{
				...
				if (mat != null || sdr != null || tex != null)
				{
                    ...
                    verticesCount += (w.geometry.m_vertexnum>0)? w.geometry.m_vertexnum: w.geometry.verts.size;
#else
	...
	bool FillDrawCall (UIDrawCall dc)
	{
		if (dc != null)
		{
			dc.isDirty = false;
			int count = 0;
            //Profiler.BeginSample("FillDrawCall WriteToBuffers " + gameObject.name);
			for (int i = 0; i < widgets.Count; )
			{
				...
				if (w.drawCall == dc)
				{
					if (w.isVisible && w.hasVertices)
					{
                        ...
                        //Profiler.BeginSample("w.WriteToBuffers");
            ...
            //Profiler.BeginSample("FillDrawCall UpdateGeometry 数量=" + count + gameObject.name);
			if (dc.verts.size != 0)
			{
				dc.UpdateGeometry(count);
				dc.onRender = mOnRender;
		...
		for (int i = 0, imax = widgets.Count; i < imax; ++i)
		{
			...
			if (w.panel == this && w.enabled)
			{
                ...
                if (w.UpdateGeometry(frame))
				{
					...
					if (!mRebuild)
					{
                        //Profiler.BeginSample("FindDrawCall");
    ...
    /// Calculate the offset needed to be constrained within the panel's bounds.
    /// </summary>
    public virtual Vector3 CalculateConstrainOffsetEx(Vector2 min, Vector2 max)
    {
        Vector4 cr = finalClipRegion;
        ...
        //-----------------------
        var rectSizeX = maxRect.x - minRect.x;
        var areaSizeX = maxArea.x - minArea.x;
        var rectSizeY = maxRect.y - minRect.y;
        var areaSizeY = maxArea.y - minArea.y;

        if (rectSizeX < areaSizeX)
        {
            maxRect.x += (areaSizeX - rectSizeX);
        }

        if (rectSizeY < areaSizeY)
        {
            minRect.y -= (areaSizeY - rectSizeY);
        }
        //-----------------------
        ...
        if (softBorderPadding && clipping == UIDrawCall.Clipping.SoftClip)
        {
            ...
            //-----------------------
            maxRect.x -= 2 * mClipSoftness.x;
            minRect.y += 2 * mClipSoftness.y;
            //-----------------------
        }

        //LogSystem.Warn("minRect = {0}, maxRect = {1}, minArea = {2}, maxArea = {3}", minRect, maxRect, minArea, maxArea);
        return NGUIMath.ConstrainRect(minRect, maxRect, minArea, maxArea);
    }
    ...
    public new static void CustomUpdate()
    {
        HashSet<UIPanel>.Enumerator iter = m_UIPanelSet.GetEnumerator();
        UIRect rect = null;
        while (iter.MoveNext())
        {
            rect = iter.Current;
    ...
    static HashSet<UIPanel> m_UIPanelSet = new HashSet<UIPanel>();

    protected override void Start()
    {
        m_UIPanelSet.Add(this);
        mStarted = true;
        OnInit();
        OnStart();
    }
    void OnDestroy()
    {
        m_UIPanelSet.Remove(this);
        if (_mChildren != null)
        {
            _mChildren.Clear();
            sCacheChildrenList.Add(_mChildren);
        }
#if USE_AUTO_RENDER_QUEUE
		if (mAutoRenderQueueList != null)
        {
            mAutoRenderQueueList.Clear();
            mAutoRenderQueueList = null;
        }
#endif
	}
...
#if OPTIMISE_GC_XY_20190123
    public class CompareFuncCls : IComparer<UIPanel>
    {
        public int Compare(UIPanel a, UIPanel b)
        {
            if (a != b && a != null && b != null)
            {
                if (a.mDepth < b.mDepth) return -1;
    ...
    public static CompareFuncCls CompareFuncComp = new CompareFuncCls();

#endif
#if USE_AUTO_RENDER_QUEUE
    
	public void AddAutoRenderQueue(AutoRenderQueue arq)
    {
        if (mAutoRenderQueueList == null)
        {
            mAutoRenderQueueList = new List<AutoRenderQueue>();
        }
        mAutoRenderQueueList.Add(arq);
    }

	
	public void RemoveAutoRenderQueue(AutoRenderQueue arq)
    {
        if (mAutoRenderQueueList != null)
        {
			mAutoRenderQueueList.Remove(arq);
        }
    }

	
	private int UpdateRenderQueueForAutoRenders()
    {
		int offset = 0;
        if (mAutoRenderQueueList != null)
        {
            for (int i = 0; i < mAutoRenderQueueList.Count; i++)
            {
                AutoRenderQueue arq = mAutoRenderQueueList[i];
                if (arq != null && arq.isActiveAndEnabled)
                {
					offset = offset + arq.GetRenderQueueOffset();
				}
            }
        }
		return offset;
