...
public class UICamera : CacheBehaviour
{
    
    public static Camera GetMainUICamera()
    {
        int index = UICamera.list.size - 1;
        if(index >= 0 && UICamera.list[index] != null)
        {
            return UICamera.list[index].gameObject.GetComponent<Camera>();
        }
        else
        {
            return null;
	...
	static private GetAnyKeyFunc GetAnyKeyDown; //lua wrap文件生成时 形参为空会有错误， TODO：修改lua工具 by liuzhen 2015-7-13 10:27:20
    ...
    static BetterList<DepthEntry> mHits2D = new BetterList<DepthEntry>();
    ...
    RaycastHit2D[] a = new RaycastHit2D[16];
    static public GameObject TryRaycast2D()
    {
        var inPos = Input.mousePosition;
        for (int i = 0; i < list.size; ++i)
        {
            UICamera cam = list.buffer[i];
            ...
            //int hitNum = Games.TLBB.Util.GameUtil.GetRayIntersectionAllNonAlloc(ray, dist, mask, out hits);
            int hitNum = 0;
            if (hitNum > 1)
            {
                for (int b = 0; b < hitNum; ++b)
                {
                ...
                mHits2D.Sort(delegate (DepthEntry r1, DepthEntry r2) { return r2.depth.CompareTo(r1.depth); });
...
//                         lastWorldPosition = mHits[b].point;
	...
	static bool IsVisible (Vector3 worldPoint, GameObject go)
	{
		...
		while (panel != null)
		{
			if (panel.IsHide || !panel.IsVisible(worldPoint)) return false;
...
#endif
	{
		...
		while (panel != null)
		{
			if (panel.IsHide || !panel.IsVisible(de.point)) return false;
			panel = panel.parentPanel;
		}
		return true;
	}
	...
	static public void Notify (GameObject go, string funcName, object obj)
	{
        ...
        {
            //if (funcName == "OnPress" && null != obj && false == (bool)obj )
            //{
            //    LuaManager.Instance.CallLuaCloseAndStopRecord(go.name);
            //}
    ...
    static public Ray GetRayByCamera(UICamera cam)
    {
        if (cam == null || currentTouch == null)
            return new Ray();
        return cam.cachedCamera.ScreenPointToRay(currentTouch.pos);
    }

    static public void SetCameraTransparencyMode(UICamera camera, int mode)
    {
        if (camera == null)
            return;
        if (mode > (int)UnityEngine.TransparencySortMode.Orthographic)
            return;
        if (mode < (int)UnityEngine.TransparencySortMode.Perspective)
            return;
        camera.cachedCamera.transparencySortMode = (UnityEngine.TransparencySortMode)mode;
        camera.cachedCamera.ResetProjectionMatrix();
    }
    ...
    static private GetTouchCountCallback GetInputTouchCount; //lua wrap文件生成时 形参为空会有错误， TODO：修改lua工具 by liuzhen 2015-7-13 10:27:20
	...
	public void ProcessTouches ()
	{
		...
		for (int i = 0; i < count; ++i)
		{
            ...
            if (currentTouch==null)
            {   // currentTouch空指针异常修正  
                // BUG: https://tapd.tencent.com/10135421/bugtrace/bugs/view?bug_id=1010135421054260397&url_cache_key=4e9f23f4c7fe8060d625ce6519791b83
                Debug.LogError(string.Format("currentTouch==null!  currentTouchID={0}", currentTouchID));
		        continue;
		    }
...
#if UNITY_IPHONE
            // all true to simulate heavy press state.
            if (Touch3DExt.GetPressure(fingerId) > 4.0f)
            {
                Notify(currentTouch.current, "OnPressure", true);
	...
	void ProcessPress (bool pressed, float click, float drag)
	{
		...
		if (pressed)
		{
            ...
            //CEventSender.SendEvent(LuaEvent.Input_OnPress);
            //EventManager.SendEvent(LuaEvent.Input_OnPress);
#if !UNITY_EDITOR
            float endGCValue = (GC.GetTotalMemory(false) / (1048576.0f));
            ResourceProxyManager.Instance.TryOnPressCollect(Math.Abs(endGCValue - beginGCValue));
#endif
            if (mTooltip != null) ShowTooltip(null);
		...
		if (currentTouch.pressed != null)
		{
				...
				currentTouch.totalDelta.sqrMagnitude < drag))
			{
				...
				if (currentTouch.clickNotification != ClickNotification.None && currentTouch.pressed == currentTouch.current)
				{
                    ...
                    if (currentTouch.pressed != null)
                    {
                        string objName = currentTouch.pressed.name;
                        if (objName == null)
                        {
                            objName = "";
                        }
                        //CEventSender.SendEvent(LuaEvent.OnPressReleaseBeforeClick, objName);
                        //LuaManager.Instance.OnPressReleaseBeforeClick(objName);
                    }
	...
	{
        //PlayerActionManager.Instance.ForceRefreshRecentTouchTime();
