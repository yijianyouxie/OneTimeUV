...
public class UIPopupList : UIWidgetContainer
{
	...
	Vector3 GetHighlightPosition ()
	{
        ...
        BoxCollider bc = mHighlightedLabel.gameObject.GetComponent<BoxCollider>();

        if (bc != null)
        {
            float scaleFactor = atlas.pixelSize;
            float offsetX = (bc.center.x - bc.size.x / 2.0f) * scaleFactor;
            float offsetY = (bc.center.y + bc.size.y / 2.0f) * scaleFactor;
            //float scaleFactor = atlas.pixelSize;
            //float offsetX = (sp.borderLeft + padding.x - mBackground.border.x) * scaleFactor;
            //float offsetY = (sp.borderTop + padding.y - mBackground.border.y) * scaleFactor;
            ...
            //mHighlight.active = true;
        }
        else
        {
			mHighlight.gameObject.SetActive (false);
            //mHighlight.active = false;
        }
        return Vector3.zero; ;
	}
	...
	public void Show ()
	{
		if (enabled && NGUITools.GetActive(gameObject) && mChild == null && atlas != null && isValid && items.Count > 0)
		{
			...
			for (int i = 0, imax = labels.Count; i < imax; ++i)
			{
				...
				bc = lbl.GetComponent<BoxCollider>();
			...
			mBackground.height = Mathf.RoundToInt(-y);
            ...
            if (bc != null)
            {
                mHighlight.width = Mathf.RoundToInt(bc.size.x);
                mHighlight.height = Mathf.RoundToInt(bc.size.y);
            }
            //mHighlight.width = Mathf.RoundToInt(w - bgPadding.x * 2.0f);
            //mHighlight.height = Mathf.RoundToInt(h + padding.y);
