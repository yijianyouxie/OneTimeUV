...
public class UIScrollView : CacheBehaviour
{
    ...
    public const float UIScrollViewEpsilon = 0.0001f;
	...
	public float momentumAmount = 100f;
	...
	protected virtual bool shouldMove
	{
		get
		{
            ...
            //以下可能出现浮点误差 做处理 2017-3-25 17:59:47 by liuzhen
			if (canMoveHorizontally)
			{
                if (b.min.x - (clip.x - hx) < -UIScrollViewEpsilon)
                {
                    return true;
                }
                if (b.max.x - (clip.x + hx) > UIScrollViewEpsilon)
                {
                    return true;
                }
			}
			...
			{
                if (b.min.y - (clip.y - hy) < -UIScrollViewEpsilon)
                {
                    return true;
                }
                if (b.max.y - (clip.y + hy) > UIScrollViewEpsilon)
                {
                    return true;
                }
			}
	...
	public bool RestrictWithinBounds (bool instant, bool horizontal, bool vertical)
	{
        ...
        Vector3 constraint;
        constraint = mPanel.CalculateConstrainOffset(b.min, b.max);
    ...
    //fix change achivement tab error which scrollview will move some distense when chang tab;
