...
public class UIWidgetInspector : UIRectEditor
{
	...
	static public void DrawInspectorProperties (SerializedObject so, UIWidget w, bool drawColor)
	{
		...
		if (NGUIEditorTools.DrawHeader("Widget"))
		{
            ...
            DrawSelfVertex(so, w, type == PrefabType.Prefab);
            DrawMaxSize(so, w, type == PrefabType.Prefab);
    ...
    static void DrawSelfVertex(SerializedObject so, UIWidget w, bool isPrefab)
    {
        if (isPrefab) return;
        ...
        GUILayout.BeginHorizontal();
        {
            EditorGUILayout.PrefixLabel("Self Vertex");
            NGUIEditorTools.DrawProperty("", so, "bLocalVertexBuffer", GUILayout.MinWidth(20f));
        }
        GUILayout.EndHorizontal();
    }
