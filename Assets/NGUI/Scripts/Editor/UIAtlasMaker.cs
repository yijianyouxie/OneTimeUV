	...
	void OnGUI ()
	{
		...
		else
		{
			...
			if (create)
			{
				...
				if (!string.IsNullOrEmpty(path))
				{
					...
					string matPath = path.Replace(".prefab", "_m.mat"); //altas分别命名 material文件命名为_m
					replace = true;
		...
		if (spriteList.Count > 0)
		{
			NGUIEditorTools.DrawHeader("Sprites", true);
			{
				...
				{
					//EditorTool.TLAssetPostprocessor.bIsProcessorOn = true;
					foreach (Texture tex in textures)
					{
					...
					if (NGUISettings.atlas.texture != null)
					{
						string path = AssetDatabase.GetAssetPath(NGUISettings.atlas.texture.GetInstanceID());
						TextureImporter ti = AssetImporter.GetAtPath(path) as TextureImporter;
						if (ti != null)
						{
							TextureImporterSettings settings = new TextureImporterSettings();
