...
public class UIRectEditor : Editor
{
    ...
    protected bool ForceSide = false;
	...
	public override void OnInspectorGUI ()
	{
		...
		DrawFinalProperties();
        if(EditorGUILayout.Toggle("ForceSide", ForceSide) != ForceSide)
        {
            ForceSide = !ForceSide;
            if (serializedObject.isEditingMultipleObjects)
            {
                ...
                {
                    (obj as UIRect).ForceSide = ForceSide;
                }
            }
            else
            {
                (target as UIRect).ForceSide = ForceSide;
            }
            //GameManager.LandscapeLeft = GameManager.LandscapeLeft;
        }
