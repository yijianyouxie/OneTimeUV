...
{
    public static bool ScreenAdapter = false;
...
//    }
	...
	{
        //if(layer == GameDefine_GlobalVar.Art2Layer)
        //{
        //    layer = GameDefine_GlobalVar.UILayer; //修正层级 art2 = UI by liuzhen 2016-8-23 20:52:15
        //}
	...
	static public void AddChildSelf(GameObject parent, GameObject self)
	{
		GameObject go = self;
		if (go != null && parent != null)
		{
	...
	static public void NormalizePanelDepths ()
	{
		...
		if (size > 0)
		{
            ...
            Array.Sort(list, UIPanel.CompareFuncComp);
#else
	...
	static public UIPanel CreateUI (Transform trans, bool advanced3D, int layer)
	{
		...
		if (trans != null)
		{
			...
			else
			{
                ...
                {
                    string errorLog = "当前组件父节点为空:" + "\n";
                    errorLog += "缺少Parent的Obj对象名称为" + trans.name;
                    //LogSystem.Error(errorLog);
                }

				trans.parent = panel.transform;
				trans.localScale = Vector3.one;
				trans.localPosition = Vector3.zero;
                trans.gameObject.layer = panel.gameObject.layer;
...
//#endif
    ...
    static public int GetActiveChildCount(Transform t, bool recursion = true)
    {
        int count = 0;
        for (int i = 0; i < t.childCount; i++)
        {
            var trans = t.GetChild(i);
            if (trans.gameObject.activeSelf)
            {
                count++;
                if (recursion)
                {
                    count += GetActiveChildCount(trans, recursion);
                }
	...
	static public Vector3[] GetSides (this Camera cam, float depth, Transform relativeTo,bool ForceSide = false)
	{
#if UNITY_4_3 || UNITY_4_5 || UNITY_4_6
...
#endif
		{
            ...
            {
                x0 += 2 * os * RectX.x;
                x1  = 2 * os * (RectX.x + RectX.width- 0.5f);
                y0 += 2 * os * RectX.y;
                y1  = 2 * os * (RectX.y + RectX.height - 0.5f);
            }
            ...
            x0 *= aspect / (RectX.width / RectX.height);
            x1 *= aspect / (RectX.width / RectX.height);
	...
	static public Vector2 screenSize
	{
		get
		{
			...
			if (mSizeFrame != frame || !Application.isPlaying)
			{
                ...
                //if(Application.isPlaying)
                //{
                //    mGameSize.x = mGameSize.x * GameManager.RectX.width;
                //    mGameSize.y = mGameSize.y * GameManager.RectX.height;
                //}
            }
			return mGameSize;
		}
	...
	static public Vector2 screenSize
    {
        get
        {
            ...
            return new Vector2(Screen.width * GameManager.RectX.width, Screen.height * GameManager.RectX.height);
        }
    }
    ...
    static public Bounds CalcWidgetBounds(GameObject go)
    {
        UIWidget[] widgets = go.GetComponentsInChildren<UIWidget>();
        ...
        if (go != null)
        {
            ...
            {
                Vector3[] sides = widgets[i].GetSides(go.transform);
                for (int j = 0; j < sides.Length; j++)
                {
                    bound.Encapsulate(sides[j]);
                }
            }
        }
    ...
    public static void MoveToScreen(GameObject go, bool isShow)
    {
        if (go == null)
        {
            return;
        ...
        {
            go.transform.localPosition = new Vector3(11000, 0, 0);
        }

    }

    public static float GetCurrentOrthographicSize()
    {
        return UIRoot.CurrentOrthographicSize;
    }

    public static void AdjustLocalScale2OrthographicSize(GameObject go)
    {
        if (!ScreenAdapter)
        {
            return;
        ...
        var s = UIRoot.CurrentOrthographicSize;
        go.transform.localScale = new Vector3(s,s,s);

#if GAMEDEBUG && !UNITY_EDITOR
        //LogSystem.Error("AdjustLocalScale2OrthographicSize, {0}, {1}", go.name, s);
#endif
