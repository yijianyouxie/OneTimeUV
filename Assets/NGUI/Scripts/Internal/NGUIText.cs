...
static public class NGUIText
{
	...
	static public void Update (bool request)
	{
...
#else
                if (dynamicFont != null && !dynamicFont.GetCharacterInfo(')', out mTempChar, finalSize, fontStyle) || mTempChar.maxY == 0f)
                {
                    if (dynamicFont != null)
                    {
                        dynamicFont.RequestCharactersInTexture("A", finalSize, fontStyle);
                    ...
                    if (dynamicFont != null && !dynamicFont.GetCharacterInfo('A', out mTempChar, finalSize, fontStyle))
                    {
                        baseline = 0f;
                        return;
                    }
        ...
        catch (System.Exception ex)
        {
            ...
            string name = "NULL";
            int len = -1;
            if (dynamicFont != null && dynamicFont.characterInfo != null)
            {
                len = dynamicFont.characterInfo.Length;
            }

            if (dynamicFont != null)
            {
                name = dynamicFont.name;
            }

            if (dynamicFont != null && dynamicFont.material != null && dynamicFont.material.mainTexture != null)
            {
                w = dynamicFont.material.mainTexture.width;
                h = dynamicFont.material.mainTexture.height;
            }
            
            //GameUtil.LogAndReportException(ex, string.Format("NGUIText.Update, name:{0}, w:{1}, h:{2}, len:{3}", name, w, h, len));
        }
    ...
    static public BMSymbol GetSymbol(StringBuilder text, int index, int textLength)
    {
        return (bitmapFont != null) ? bitmapFont.MatchSymbol(text, index, textLength) : null;
    }
    ...
    static public Color ParseColor24(StringBuilder text, int offset)
    {
        int r = (NGUIMath.HexToDecimal(text[offset]) << 4) | NGUIMath.HexToDecimal(text[offset + 1]);
        int g = (NGUIMath.HexToDecimal(text[offset + 2]) << 4) | NGUIMath.HexToDecimal(text[offset + 3]);
        int b = (NGUIMath.HexToDecimal(text[offset + 4]) << 4) | NGUIMath.HexToDecimal(text[offset + 5]);
    ...
    static public Color ParseColor32(StringBuilder text, int offset)
    {
        int r = (NGUIMath.HexToDecimal(text[offset]) << 4) | NGUIMath.HexToDecimal(text[offset + 1]);
        int g = (NGUIMath.HexToDecimal(text[offset + 2]) << 4) | NGUIMath.HexToDecimal(text[offset + 3]);
        int b = (NGUIMath.HexToDecimal(text[offset + 4]) << 4) | NGUIMath.HexToDecimal(text[offset + 5]);
    ...
    static public bool ParseSymbol(StringBuilder text, ref int index, BetterList<Color> colors, bool premultiply,
    ref int sub, ref bool bold, ref bool italic, ref bool underline, ref bool strike, ref bool ignoreColor)
    {
        ...
        if (text[index + 3] == ']')
        {
            char tag1 = text[index + 1];
            char tag2 = text[index + 2];
        ...
        if (text[index + 4] == ']')
        {
            char tag1 = text[index + 1];
            char tag2 = text[index + 2];
            char tag3 = text[index + 3];

            if (tag1 == 's' && tag2 == 'u' && tag3 == 'b')
            {
                sub = 1;
                index += 5;
                return true;
            }
            else if (tag1 == 's' && tag2 == 'u' && tag3 == 'p')
            {
                sub = 2;
                index += 5;
                return true;
        ...
        if (text[index + 5] == ']')
        {
            ...
            char tag2 = text[index + 2];
            char tag3 = text[index + 3];
            char tag4 = text[index + 4];

            if (tag1 == '/' && tag2 == 's' && tag3 == 'u' && tag4 == 'b')
            {
                sub = 0;
                index += 6;
                return true;
            }
            else if (tag1 == '/' && tag2 == 's' && tag3 == 'u' && tag4 == 'p')
            {
                sub = 0;
                index += 6;
                return true;
            }
            else if (tag1 == '/' && tag2 == 'u' && tag3 == 'r' && tag4 == 'l')
            {
                index += 6;
                return true;
            }
        ...
        if (text[index + 7] == ']')
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                if (NGUIMath.IsColorChar(text[index + 1 + i]) == false)
                {
                    bIsColor = false;
                    break;
                }
	...
	static public string StripSymbols (string text)
	{
		if (text != null)
		{
            //StringBuilder sb = Games.TLBB.Util.StringHelper.PoolNew(text);
            string sb = "";
            for (int i = 0, imax = sb.Length; i < imax; )
			{
                char c = sb[i];
				...
				if (c == '[')
				{
                    ...
                    if (ParseSymbol(sb, ref retVal, null, false, ref sub, ref bold, ref italic, ref underline, ref strikethrough, ref ignoreColor))
					{
                        sb = sb.Remove(i, retVal - i);
                        imax = sb.Length;
						continue;
					}
				}
            ...
            //return Games.TLBB.Util.StringHelper.PoolRet(ref sb);
		}
        return text;
	}


    static public StringBuilder StripSymbolsSB(string text)
    {
        if (text != null)
        {
            //StringBuilder sb = Games.TLBB.Util.StringHelper.PoolNew(text);
	...
	static public Vector2 CalculatePrintedSize (string text)
	{
		...
		if (!string.IsNullOrEmpty(text))
		{
            ...
            if (encoding)
            {
                sb = StripSymbolsSB(text);
            }
            else
            {
                //sb = Games.TLBB.Util.StringHelper.PoolNew(text);
            }
            ...
            int textLength = sb.Length, ch = 0, prev = 0;
			...
			for (int i = 0; i < textLength; ++i)
			{
                ...
                BMSymbol symbol = useSymbols ? GetSymbol(sb, i, textLength) : null;
            ...
            //Games.TLBB.Util.StringHelper.PoolDel(ref sb);
		}
		return v;
	}
	...
	static public bool WrapText (string text, out string finalText, bool keepCharCount, bool wrapLineColors)
	{
		...
		if (wrapLineColors) //mColors.Add(c);
        {
            int size;
            size = mColors.size;
            mColors.Allocate(mColors.size += 1);
		...
		for (; offset < textLength; ++offset)
		{
			...
			if (ch == '\n')
			{
                ...
                if (start < offset) sb.Append(text, start, offset - start + 1);
				else sb.Append(ch);
        ...
        if (start < offset) sb.Append(text, start, offset - start);
        if (wrapLineColors && mColors.size > 0) sb.Append("[-]");
	...
	static public void Print (string text, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			{
				Color fc;
                ignoreColor = true; //始终忽略颜色 然后默认色是底色 2017-4-5 2017-4-5 22:45:40 by liuzhen
                if (ignoreColor)
                {
                    if (mColors.size == 1)
                    {
