...
public abstract class UIRect : CacheBehaviour
{
	[System.Serializable]
	public class AnchorPoint
	{
		...
		public Vector3[] GetSides (Transform relativeTo,bool ForceSide = false)
		{
			if (target != null)
			{
				if (rect != null) return rect.GetSides(relativeTo, ForceSide);
    ...
    protected BetterList<UIRect> _mChildren;
    ...
    protected BetterList<UIRect> mChildren
    {
        get
        {
            if(_mChildren == null)
            {
                _mChildren = this.GetLisRect();
            }
            return _mChildren;
        }
    }
    protected static int sCacheRectCount = 1000;
    public static List<BetterList<UIRect>> sCacheChildrenList = new List<BetterList<UIRect>>(sCacheRectCount);
    /// <summary>
    /// 获取一个缓存Childrenlist
    /// </summary>
    /// <returns></returns>
    protected BetterList<UIRect> GetLisRect()
    {
        if (sCacheChildrenList.Count == 0)
        {
            //初始化
            for (int i = 0; i < sCacheRectCount; i++)
            {
                sCacheChildrenList.Add(new BetterList<UIRect>());
            }
        }
        int count = sCacheChildrenList.Count;
        BetterList<UIRect> item = sCacheChildrenList[count - 1];
        sCacheChildrenList.RemoveAt(count - 1);
        return item;
    }
	...
	public virtual Vector3[] GetSides (Transform relativeTo,bool ForceSide = false)
	{
		if (anchorCamera != null) return mCam.GetSides(cameraRayDistance, relativeTo,ForceSide);
    ...
    public static int uiOpenFrame = -1;
    
    public static int updateCount = 50;
    ...
    public static List<UIRect> m_UIRectList = new List<UIRect>();

	protected virtual void Start ()
	{
        m_UIRectList.Add(this);
		mStarted = true;
		OnInit();
		OnStart();
    ...
    [ContextMenu("ResetAndUpdateAnchors")]
