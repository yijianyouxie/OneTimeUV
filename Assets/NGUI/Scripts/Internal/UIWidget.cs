...
public class UIWidget : UIRect
{
    ...
    [HideInInspector][SerializeField] public bool bLocalVertexBuffer = false;
    ...
    private UIPanel parentPanel = null;

    private UIGeometry _geometry = null;
    ...
    public UIGeometry geometry
    {
        get
        {
            if(_geometry == null)
            {
                _geometry = this.GetUIGeometry();
                _geometry.m_pUIWidget = this;
            }
            return _geometry;
        }
        set
    ...
    protected static Vector3[] smCorners = new Vector3[4];
    [System.NonSerialized] protected Vector3[] mCorners = smCorners;

    protected static int sCacheCount = 1000;
    public static List<UIGeometry> sCacheGeometryList = new List<UIGeometry>(sCacheCount);
    /// <summary>
    /// 获取一个缓存Geometry
    /// </summary>
    /// <returns></returns>
    protected UIGeometry GetUIGeometry()
    {
        if(sCacheGeometryList.Count == 0)
        {
            //初始化
            for (int i = 0; i < sCacheCount; i++)
            {
                sCacheGeometryList.Add(new UIGeometry());
            }
        }
        int count = sCacheGeometryList.Count;
        UIGeometry item = sCacheGeometryList[count - 1];
        sCacheGeometryList.RemoveAt(count - 1);
        return item;
    }
    ...
    static public int PanelDistanceCompareFunc (UIWidget left, UIWidget right)
    {
        if (UnityEngine.Camera.main == null)
            return 0;

        float d1 = Vector3.Distance(left.gameObject.transform.position, UnityEngine.Camera.main.gameObject.transform.position);
        float d2 = Vector3.Distance(right.gameObject.transform.position, UnityEngine.Camera.main.gameObject.transform.position);
	...
	protected override void OnStart ()
	{
		...
		{
            Debug.LogError(string.Format("Widgets and panels should not be on the same object!Widget must be a child of the panel. {0}", NGUITools.GetHierarchy(gameObject)));
		}
		else if (!Application.isPlaying && GetComponents<UIWidget>().Length > 1)
		{
			Debug.LogError(string.Format("You should not place more than one widget on the same object. Weird stuff will happen. {0}", NGUITools.GetHierarchy(gameObject)));
		}
#endif
		CreatePanel();
	}
	...
	protected override void OnAnchor ()
	{
		...
		{
			Vector3[] sides = leftAnchor.GetSides(parent,ForceSide);
    ...
    public static new void CustomUpdate()
    {
        ...
        int count = m_UIWidgetList.Count;
        if (Time.renderedFrameCount - uiOpenFrame > 10)
        {
            for (int i = 0; i < updateCount && index < count; ++i, ++index)
            {
                widget = m_UIWidgetList[index];
    ...
    public static List<UIWidget> m_UIWidgetList = new List<UIWidget>();
    protected override void Start()
    {
        m_UIWidgetList.Add(this);
        mStarted = true;
        OnInit();
        OnStart();
    }
    ...
    void OnDestroy()
    {
        ...
        {
            _geometry.Clear();
            _geometry.m_pUIWidget = null;
            sCacheGeometryList.Add(_geometry);
            _geometry = null;
        }
        if(_mChildren != null)
        {
        ...
        RemoveFromPanel();
        m_UIWidgetList.Remove(this);

        //AutoRenderQueue = null;
    }
	...
	public bool UpdateGeometry (int frame)
	{
		...
		if (mChanged)
		{
			...
			if (mIsVisibleByAlpha && finalAlpha > 0.001f && shader != null)
			{
...
#else
                    geometry.mRtpVerts.size = 0;
#endif

                    geometry.verts.size = 0;
                    geometry.uvs.size = 0;
                    geometry.cols.size = 0;

                    //Profiler.BeginSample("OnFill");
                    OnFill(geometry.verts, geometry.uvs, geometry.cols, geometry);
                    //Profiler.EndSample();
                }
                ...
                if (geometry.hasVertices)
				{
                    ...
                    //Profiler.BeginSample("ApplyTransform1");

                    geometry.ApplyTransform(ref mLocalToPanel);
		...
		else if (mMoved && geometry.hasVertices)
		{
            ...
            //Profiler.BeginSample("ApplyTransform2");
    ...
    virtual public int minHeight { get { return _minHeight; } set { _minHeight = value; } }
	...
	virtual public void OnFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols, UIGeometry uig = null)
	{
